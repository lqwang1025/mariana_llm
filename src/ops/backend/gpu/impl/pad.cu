...
template<typename T>
__global__ void __nchw_pad_kernel(const T* input, T* out, int32_t OC, int32_t OH, int32_t OW, T pad_value, uint32_t input_stride_1, uint32_t input_stride_2, uint32_t out_stride_1, uint32_t out_stride_2, uint32_t pl, uint32_t pr, uint32_t pt, uint32_t pb, uint32_t pf, uint32_t pbk) {
    int32_t index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= OC*OW*OH) return;
    uint32_t _idx = index;
    uint32_t w = _idx%OW;
    _idx /= OW;
    uint32_t h = _idx%OH;
    _idx /= OH;
    uint32_t c = _idx;
    uint32_t oidx = c*out_stride_1+h*out_stride_2+w;
    if (w < pl || (OW-pr-1) < w) {
        out[oidx] = pad_value;
        return;
    }
    ...
    uint32_t ih = h-pt;
    uint32_t iw = w-pl;
    uint32_t iidx = ic*input_stride_1+ih*input_stride_2+iw;
    out[oidx] = input[iidx];
}
...
void nchw_pad(SchedParam sched_param, const Tensor& input, Tensor& out, uint32_t padding[6], float pad_value, CUDAContext* cuda_ctx) {
    if (out.dtype().match<float>()) {
        ...
        __nchw_pad_kernel<float><<<get_cuda_gridsize(ooffset, CUDA_PAD_BLOCK_SIZE),
            CUDA_PAD_BLOCK_SIZE, 0, cuda_ctx->stream(sched_param.id_thread)>>>(input_ptr, out_ptr, OC, OH, OW, pad_value, istride_1, istride_2, ostride_1, ostride_2, pl, pr, pt, pb, pf, pbk);
