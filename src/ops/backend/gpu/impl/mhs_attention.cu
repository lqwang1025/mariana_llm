...
__global__ void __mhs_mask_attention_float32_kernel(const float* q_ptr, const float* k_ptr, const float* v_ptr, const float* mask_ptr, uint32_t distance, uint32_t QT, uint32_t KT, uint32_t VT, uint32_t n_head, uint32_t head_size, uint32_t q_stride_0, uint32_t q_stride_1, uint32_t k_stride_0, uint32_t k_stride_1, uint32_t mask_dim_1, uint32_t mask_stride_1, uint32_t mask_stride_2, uint32_t v_stride_0, uint32_t v_stride_1, uint32_t out_stride_0, uint32_t out_stride_1, float* out_ptr) {
    ...
    for (int32_t h = 0; h < n_head; ++h) {
        ...
        for (uint32_t t2 = 0; t2 < KT; ++t2) { // 52
            ...
            for (int i = 0; i < head_size; ++i) { // 64
                float q = q_ptr[n*q_stride_0+t*q_stride_1+h*head_size+i];
                float k = k_ptr[n*k_stride_0+t2*k_stride_1+h*head_size+i];
                val += q*k;
            }
        ...
        for (int32_t i = 0; i < head_size; ++i) {
            out_ptr[n*out_stride_0+t*out_stride_1+h*head_size+i] = 0;
        }
        for (uint32_t t2 = 0; t2 < VT; ++t2) {
            for (int32_t i = 0; i < head_size; ++i) {
                float value = v_ptr[n*v_stride_0+t2*v_stride_1+h*head_size+i];
                out_ptr[n*out_stride_0+t*out_stride_1+h*head_size+i] += value*_qk_dot[t2];
            }
        }
    } // n_head
}

__global__ void __mhs_attention_float32_kernel(const float* q_ptr, const float* k_ptr, const float* v_ptr, uint32_t distance, uint32_t QT, uint32_t KT, uint32_t VT, uint32_t n_head, uint32_t head_size, uint32_t q_stride_0, uint32_t q_stride_1, uint32_t k_stride_0, uint32_t k_stride_1, uint32_t v_stride_0, uint32_t v_stride_1, uint32_t out_stride_0, uint32_t out_stride_1, float* out_ptr) {
    int32_t index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= QT*distance) return;
    int32_t t = index % QT;
    int32_t n = index / QT;
...
void mhs_attention(SchedParam sched_param, const Tensor& Q, const Tensor& K, const Tensor& V, Tensor& out, int32_t n_head, int32_t head_size, CUDAContext* cuda_ctx) {
    if (out.dtype().match<float>()) {
        ...
        float* out_ptr  = out.unsafe_ptr<float>(sched_param.this_thread_begin_index()*o_offset);
        uint32_t distance = sched_param.this_thread_end_index() - sched_param.this_thread_begin_index();
        __mhs_attention_float32_kernel<<<get_cuda_gridsize(distance*QT, CUDA_ATTN_BLOCK_SIZE),
            CUDA_ATTN_BLOCK_SIZE, 0, cuda_ctx->stream(sched_param.id_thread)>>>(q_ptr, k_ptr, v_ptr, distance, QT, KT, VT, n_head, head_size, Q.stride_at(0), Q.stride_at(1), K.stride_at(0), K.stride_at(1), V.stride_at(0), V.stride_at(1), out.stride_at(0), out.stride_at(1), out_ptr);
        cuda_ctx->stream_sync(cuda_ctx->stream(sched_param.id_thread));
    } else {
        MLOG(FATAL)<<"Mhs attention unsupport datatype:"<<out.dtype().name();
    }
