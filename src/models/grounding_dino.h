...
public:
    ...
    virtual bool make_graph(const char* dir_path, ExeContext& context)override;
private:
    ...
    void _post_process(const Tensor& bbox, const Tensor& scores, const Tensor& probs, const std::vector<int32_t>& token, AIResult& result);
