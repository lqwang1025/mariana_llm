...
__global__ void __mhs_mask_attention_float32_kernel(const float* q_ptr, const float* k_ptr, const float* v_ptr, const float* mask_ptr, uint32_t NC, uint32_t n_head, uint32_t head_size, uint32_t q_stride_1, uint32_t k_stride_1, uint32_t mask_dim_1, uint32_t mask_stride_1, uint32_t mask_stride_2, uint32_t v_stride_1, uint32_t out_stride_1, float* out_ptr) {
...
__global__ void __mhs_attention_float32_kernel(const float* q_ptr, const float* k_ptr, const float* v_ptr, const float* mask_ptr, uint32_t NC, uint32_t n_head, uint32_t head_size, uint32_t q_stride_1, uint32_t k_stride_1, uint32_t mask_dim_1, uint32_t mask_stride_1, uint32_t mask_stride_2, uint32_t v_stride_1, uint32_t out_stride_1, float* out_ptr) {
    int32_t t = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (t >= NC) return;
    float scale = 1.f/sqrtf(head_size);
    float _qk_dot[256] = {0.f};
...
void mhs_mask_attention(SchedParam sched_param, const Tensor& Q, const Tensor& K, const Tensor& V, const Tensor& mask, Tensor& out, int32_t n_head, int32_t head_size, CUDAContext* cuda_ctx) {
    if (out.dtype().match<float>()) {
        uint32_t T = Q.dim_at(1); // [batch, T, C]
        ...
        __mhs_mask_attention_float32_kernel<<<get_cuda_gridsize(T, CUDA_ATTN_BLOCK_SIZE),
            CUDA_ATTN_BLOCK_SIZE, 0, cuda_ctx->stream(sched_param.id_thread)>>>(q_ptr, k_ptr, v_ptr, mask_ptr, T, n_head, head_size, Q.stride_at(1), K.stride_at(1), mask.dim_at(1), mask.stride_at(1), mask.stride_at(2), V.stride_at(1), out.stride_at(1), out_ptr);
    } else {
        MLOG(FATAL)<<"Add unsupport datatype:"<<out.dtype().name();
    }
}
